                    ...
                    <div class="w-full bg-white">
                        ...
                        <div class="flex flex-col">
                            ...
                            <div class="border rounded-md p-3  my-3">
                                ...
                                <div class="flex justify-between items-center">
                                    <div>
                                        <a href="{{ route('comment.edit',['article'=>$article->id,'comment'=>$comment->id]) }}" class="text-gray-500 hover:text-gray-700 mr-4"><i class="far fa-thumbs-up"></i>
                                            Modifier</a>

                                        <a href="{{ route('comment.delete',['id'=>$comment->id]) }}" class="text-gray-500 hover:text-gray-700"><i class="far fa-comment-alt"></i>
                                            Supprimer</a>

                                        {{-- <a
                                            href="{{ route('comment.edit',['article'=>$article->id,'comment'=>$comment->id]) }}"
                                        class="text-gray-500 hover:text-gray-700 mr-4"><i class="far fa-thumbs-up"></i>
                                        Modifier</a>
                                        <a href="{{ route('comment.delete',['id'=>$comment->id]) }}" class="text-gray-500 hover:text-gray-700"><i class="far fa-comment-alt"></i>
                                            Supprimer</a> --}}
                                    </div>
                                    {{-- <div class="flex items-center">
                                        <a href="#" class="text-gray-500 hover:text-gray-700 mr-4"><i
                        ...
                        <form method="post" action="{{ route('comment.add') }}">
                            ...
                            <div class="w-full my-2">
                                <textarea class=" rounded border leading-normal resize-none w-full h-20 py-2 px-3 font-medium  focus:outline-none" name="commentaire" placeholder='Type Your Comment' required>Merci Pour Le commentaire</textarea>
                            </div>

                            <div class="w-full flex justify-start ">
                                <input type='submit' class="px-2.5 py-1.5 rounded text-white text-sm bg-stone-950" value='Post Comment'>
                            </div>
                        </form>
            ...
            <div class="w-full md:w-4/12 px-4 mb-8">
                <div class=" px-4 py-6 rounded">
                    ...
                    <div class="flex  gap-2 flex-wrap ">
                        <span class="bg-gray-100 rounded-full px-3 py-1 text-sm font-semibold text-gray-600">{{ $article->categorie }}</span>
                ...
                <div class="mt-5">
                    ...
                    @can('acces_admin')
                    <a href="{{route('edit',['id'=>$article->id])}}" class="bg-green-500  text-white font-bold py-2 px-4 rounded mr-3">
                        Modifier l'article
                    </a>

                    <a href="{{route('delete',['id'=>$article->id])}}" class="bg-red-500 text-white font-bold py-2 px-4 rounded">
                        Supprimer l'article
                    </a>
                    @endcan

                    @foreach($article->getChambres() as $chambre)
                    <h1><b>{{ $chambre['libelle'] }}</b></h1>
                    <img src="{{ asset($chambre['image']) }}" alt="Chambre">
                    <p> Dimension: {{ $chambre['dimension'] }}</p><br>
                    @endforeach
